...
function uopt = FindOptimalInput(x0_set, N, ts, target, xObst, threshold, L, speedBound, steeringBound)
    ...
    [uopt ,fval,exitflag,output] = fmincon(@(u) Cost(x0_set,u,ts,target,L),uInit,A,b,Aeq,beq,lb,ub, @(u) ObstConstraint(x0_set,u,ts,xObst,threshold,L,polyOptions),options);
    output
